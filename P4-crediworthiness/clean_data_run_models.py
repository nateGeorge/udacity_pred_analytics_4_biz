...
def clean_data(train, test, var_types):
    ...
    '''
    drop_cols = ['Duration-in-Current-address', 'Occupation',
...
if __name__ == "__main__":
    do_stuff = True
    if do_stuff:
        # first get platform, since parallel processing with sklearn
        # and gridsearch doesn't work on windows, but does on ubuntu
        plat = sys.platform
        windows = False
        ...
        # look at relative standard deviation to find which has low variance
        train2 = train.copy()
        for c in train2.columns:
            if train2[c].dtype == 'O': # object datatype for strings
                le = LE()
                train2[c] = le.fit_transform(train2[c])

        train2.drop('Duration-in-Current-address', inplace=True, axis=1)
        train2['Age-years'].fillna(int(train2['Age-years'].mean()), inplace=True)
        rel_std = []
        for c in train2.columns:
            rel_std.append((c, variation(train2[c])))
        ...
        # parallel processing doesn't work on my windows 10...oye
        gs = GSCV(dt, param_grid=params, n_jobs=-1)
        if windows:
            gs = GSCV(dt, param_grid=params)
